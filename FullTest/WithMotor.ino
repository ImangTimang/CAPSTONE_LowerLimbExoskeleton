...
#define MPU5_CHANNEL 4
#define MPU6_CHANNEL 5

#define MOTOR_DIR_PIN 8       // Direction pin for MD20A motor driver
#define MOTOR_PWM_PIN 9       // PWM/speed pin for MD20A motor driver
#define ACCEL_THRESHOLD 500   // Adjust this based on sensitivity needs
...
void activateMotor(bool enable) {
    if (enable) {
        digitalWrite(MOTOR_DIR_PIN, HIGH);   // or LOW depending on desired direction
        analogWrite(MOTOR_PWM_PIN, 200);     // Set PWM speed (0-255)
    } else {
...
void checkThreshold(MPU6050 &mpu, uint8_t channel) {
    ...
    Serial.println(accelX_mpu5);

    // Logic for motor control based on MPU thresholds

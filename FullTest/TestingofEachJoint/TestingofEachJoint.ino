...
const unsigned long THRESHOLD_DELAY = 500; // Adjust this value for the delay in milliseconds

float potResetAngles[4] = {170.0, 170.0, 170.0, 170.0};
float potEndHigh[4] = {255.0, 240.0, 240.0, 240.0};
...
#define MPU2_CHANNEL 2
#define MPU3_CHANNEL 1
...
unsigned long lastThresholdTime[4] = {0, 0, 0, 0};
...
void stopMotor(int index) {
  ...
  lastThresholdTime[index] = millis(); // Record the time when the motor stopped
}
...
void resetToAngle() {
  for (int i = 0; i < 4; i++) {
    ...
    bool forward;
    if (i == 0 || i == 2) { // M1 and M3: Move towards higher angle
      forward = (currentAngle < targetAngle);
    } else { // M2 and M4: Move towards lower angle
      forward = (currentAngle > targetAngle);
    }
...
void loop() {
  ...
  for (int i = 0; i < 4; i++) {
    switch (currentState[i]) {
      case WAIT_FORWARD:
        if (millis() - lastThresholdTime[i] >= THRESHOLD_DELAY) {
          if (i == 0 || i == 2) { // MPU 1 & 3: Positive threshold for forward
            if (accelY[i] > mpuThresholds[i]) {
              startMotor(i, true);
              currentState[i] = FORWARD;
            } else if (accelY[i] < -mpuThresholds[i]) {
              startMotor(i, false);
              currentState[i] = REVERSE;
            }
          } else { // MPU 2 & 4: Positive threshold for reverse
            if (accelY[i] > mpuThresholds[i]) {
              startMotor(i, false);
              currentState[i] = REVERSE;
            } else if (accelY[i] < -mpuThresholds[i]) {
      ...
      case WAIT_REVERSE:
        if (millis() - lastThresholdTime[i] >= THRESHOLD_DELAY) {
          if (i == 0 || i == 2) { // MPU 1 & 3: Negative threshold for reverse
            if (accelY[i] < -mpuThresholds[i]) {
              startMotor(i, false);
              currentState[i] = REVERSE;
            } else if (accelY[i] > mpuThresholds[i]) {
          ...
          } else { // MPU 2 & 4: Negative threshold for forward
            if (accelY[i] < -mpuThresholds[i]) {
              startMotor(i, true);
              currentState[i] = FORWARD;
            } else if (accelY[i] > mpuThresholds[i]) {
      ...
      case REVERSE:
        if (potAngles[i] <= potEndLow[i] + POT_HYSTERESIS) {
          stopMotor(i);
          currentState[i] = WAIT_FORWARD;
          reachedHigh[i] = false; // Reset reachedHigh when going back
        }
        break;
      delay(53);
    }
